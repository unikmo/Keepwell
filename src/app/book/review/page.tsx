      ...
      <div className="w-full max-w-lg rounded-3xl border border-line bg-surface p-6 sm:p-8">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm leading-6 text-parchment-dim">
            Submitting creates a marketplace request. A provider and ETA appear only after a real independent provider accepts.
          </p>
        </div>
        ...
        <div className="mt-6 rounded-2xl border border-line bg-ink/35 p-5 text-sm">
          ...
          <div className="mt-4 border-t border-line/70 pt-4">
            <div className="flex items-start justify-between gap-4">
              <span className="text-parchment-dim">Fixed standard price</span>
              <span className="font-mono text-2xl text-brass">{formatServicePrice(service.customerPriceCents)}</span>
            </div>
        ...
        <p className="mt-5 text-center text-xs text-parchment-dim">
          <Link href={`/book/details?service_id=${service.id}`} className="hover:text-parchment">← Edit details</Link>
        </p>
      </div>
    </div>
  );
