...
export const metadata: Metadata = {
  ...
  description: "How Keepwell handles account, Digital Sentinel, property-access, marketplace, and provider information.",
...
export default function PrivacyPage() {
  return (
    <LegalDoc title="Privacy Policy" lastUpdated="August 11, 2026">
      <LegalSection title="1. Information we collect"><p>Keepwell may collect account information, Digital Sentinel access records, trusted-contact information, reference photos, service-request details, property-manager and brokerage information, provider onboarding and job-response information, transaction records, and technical/security data needed to operate the Service.</p></LegalSection>
      <LegalSection title="2. Digital Sentinel sensitive information"><p>Where secret storage is enabled, sensitive text such as a lockbox or keypad code is encrypted by the Keepwell application server before the resulting ciphertext is persisted to the database. The server-side encryption key is not stored in the application database. Private reference photos are stored separately in a non-public storage bucket and are presented through short-lived signed access. Keepwell does not describe this design as zero-knowledge or end-to-end encryption.</p></LegalSection>
      <LegalSection title="3. Trusted contacts"><p>A trusted contact may be recorded as a spare-key holder, emergency contact, or person who can authorize access. Recording a trusted contact does not by itself grant that person access to the member's Digital Sentinel secrets.</p></LegalSection>
      ...
      <LegalSection title="5. Information shared with providers"><p>Before acceptance, provider-visible information should be limited to what is reasonably necessary to evaluate a job. After acceptance, Keepwell may provide details reasonably necessary to perform that service. Providers do not receive standing access to Digital Sentinel secrets or unrelated customer records. Audit providers submit their report to Keepwell rather than receiving authority to sell follow-up work in the home.</p></LegalSection>
