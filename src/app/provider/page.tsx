...
export default async function ProviderDashboard({ searchParams }: { searchParams: Promise<{ notice?: string; error?: string }> }) {
  ...
  return <div className="flex min-h-screen flex-col"><Nav /><main className="flex-1 py-12"><div className="mx-auto max-w-6xl px-6">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><div className="eyebrow">Provider dashboard</div><h1 className="mt-2 font-display text-4xl text-parchment">{profile.business_name}</h1><p className="mt-2 text-sm text-parchment-dim">Verified independent provider · {[profile.city, profile.state].filter(Boolean).join(", ")}</p></div><form action={setAvailability}><input type="hidden" name="is_available" value={profile.is_available ? "false" : "true"} /><button className={`rounded-full px-5 py-2.5 text-sm font-semibold ${profile.is_available ? "border border-line text-parchment" : "bg-brass text-ink"}`}>{profile.is_available ? "Pause availability" : "Go available"}</button></form></div>
